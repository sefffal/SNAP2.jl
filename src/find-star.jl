...
function centrefit(full::AbstractArray,  xs::Tuple=(1,size(full,1)), ys::Tuple=(1,size(full,2)); std, neg=false) 
    measured = @view full[xs[1]:xs[2], ys[1]:ys[2]]
    ValType = eltype(measured)
    model(A, offset, μx, μy, x, y, std) = gauss2D(A, offset, μx, μy, std, std, x, y)
